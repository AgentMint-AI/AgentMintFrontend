...
import { ShowcaseMetadata } from "@/types/token";
...
export async function POST(req: Request) {
  ...
  try {
    const metadata: ShowcaseMetadata = await req.json();
    ...
    console.error("Error uploading showcase metadata:", error);
    return NextResponse.json(
      { error: "Failed to upload showcase metadata" },
      { status: 500 }
    );
  }
}
